...
Init:
  ...
  IniRead DestDir,      %IniFile%, Recent, Destination, %A_ScriptDir%
...
Main:
  ...
  MsgBox 64,Info,Converter software. Choosen files are converted in the SAME folder. We let you to choose a folder to move the files after each conversion. Valid input extensions = "*.wav, *.mp3, *.ogg, *.ape, *.m4a, *.aac, *.flac"
 ...
 ;Gui Add, Checkbox, hp wp 0 vGuiAlwaysOnTop gToggleOnTop , &Always on top
...
Convert( files, outFormat, delSource=false ) {
  ...
  Loop %File0% {
    ...
    If( RetCode = "Success" ) { 
      If( delSource and InExtension != outFormat ) {
		SplashTextOn,,, File converted and deleting original file.
		Sleep 1000
		SplashTextOff
        DelFile( File%A_Index% )
      }Else{
		SplashTextOn,,, File converted.
		Sleep 1000
		SplashTextOff
	  }
    }
  ...
  If( RetCode = "Success" ) { 
	MsgBox 64,Info,The process has been completed.
  }
}
